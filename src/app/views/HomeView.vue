    ...
    <section class="relative w-full h-[500px] md:h-[600px] overflow-hidden">
      ...
      <div class="absolute inset-0 z-20 flex items-center">
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div class="max-w-xl">
            <p class="text-[#F97316] font-semibold text-sm tracking-widest uppercase mb-4">{{ currentSlideData.subtitle }}</p>
      ...
      <div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
        <button
          ...
          :class="currentSlide === index ? 'bg-[#F97316]' : 'bg-white/40 hover:bg-white/60'"
    ...
    <section class="py-16 bg-white">
      <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-10">
          ...
          <router-link to="/products" class="text-sm font-semibold text-[#666] hover:text-[#F97316] transition-colors flex items-center gap-1">
            VIEW ALL
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
          </router-link>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          <router-link
            ...
            class="group bg-[#F5F5F5] rounded-lg p-6 text-center hover:bg-[#F97316] transition-all duration-300"
          >
      ...
      <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          <div v-for="stat in stats" :key="stat.label">
            <div class="font-heading font-black text-4xl text-[#F97316]">{{ stat.value }}</div>
